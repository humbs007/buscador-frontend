...
export function getFieldLabel(table, field) {
  ...
  return DB_SCHEMA?.tabelas?.[key]?.campos?.[field] || field;
}

// 🔍 Retorna nome amigável da tabela
export function getTableLabel(table) {
  const key = table?.toLowerCase?.();
  return DB_SCHEMA?.tabelas?.[key]?.nome || table?.toUpperCase();
}

// 🔄 Retorna campos mapeados por uma chave unificada ou todos os mapeamentos se sem argumento
export function getUnifiedFields(key) {
  if (key) {
    return DB_SCHEMA?.unificados?.[key] || [key];
  }
  return DB_SCHEMA.unificados;
}

// 📋 Retorna todas as chaves unificadas
export function getAllUnifiedKeys() {
  return Object.keys(DB_SCHEMA?.unificados || {});
}
